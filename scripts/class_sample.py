...
def parseArguments(myArgs=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="RGB",
        help="the mode of the PIL image to render"
    )
    parser.add_argument(
        "--skip_grid",
        action='store_true',
    ...
    parser.add_argument(
        ...
        type=str,
        default="[1,0,0,0,0,0]",
        help="the class vector used for conditioning",
    )
    parser.add_argument(
        "--from-file",
        type=str,
        nargs="?",
        help="if specified, load conditioning from this file",
    )
    parser.add_argument(
        "--from-onehot-file",
        type=str,
...
def sampleLoop(opt, model, sampler):
    ...
    # From list file
    if opt.from_file:        
        print(f"Getting class vectors from {opt.from_file}")
...
def txtMain(opt, model=None):
    ...
    if opt.from_file: 
        print(f"Getting class from captions from {opt.from_file}")
        with open(opt.from_file, "r") as f:
            data = f.read().splitlines()
            data = list(chunk(data, batch_size))
    elif opt.from_onehot_file: 
        print(f"Getting class vectors from {opt.from_onehot_file}")
        with open(opt.from_onehot_file, "r") as f:
            data = json.load(f)
            batchdata= []
            for label in data:
                batchLabel = batch_size * [label]
                batchdata.append(batchLabel)
            data = batchdata
    else:
        class_vector = json.loads(opt.class_label)
        assert class_vector is not None
        data = [batch_size * [class_vector]]

    sample_path = os.path.join(outpath, "TESTING", time.strftime("%Y-%m-%d %H-%M-%S"))
    grid_path = os.path.join(outpath, "TESTING-grids", time.strftime("%Y-%m-%d %H-%M-%S"))
    os.makedirs(sample_path, exist_ok=True)
    os.makedirs(grid_path, exist_ok=True)
    base_count = len(os.listdir(sample_path))
    grid_count = len(os.listdir(outpath)) - 1
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for label in tqdm(data, desc="data"):
                        ...
                        if opt.from_file:
                            mydict = {}
                            mydict['caption'] = label
                            c = model.get_learned_conditioning(mydict)
                        elif opt.from_onehot_file:
                            c = torch.tensor(label)
                            c = c[:, None,:]
                            c = c.to(torch.float).to(model.device)
                        ...
                        if not opt.skip_save:
                            for x_sample in x_checked_image_torch:
                                x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                                img = Image.fromarray(x_sample.astype(np.uint8), opt.mode)
                                # img = put_watermark(img, wm_encoder)
                                img.save(os.path.join(sample_path, f"{base_count:05}.png"))
                                base_count += 1
                ...
                if not opt.skip_grid:
                    ...
                    # to image
                    grid = 255. * rearrange(grid, 'c h w -> h w c').cpu().numpy()
                    img = Image.fromarray(grid.astype(np.uint8), opt.mode)
                    # img = put_watermark(img, wm_encoder)
                    img.save(os.path.join(grid_path, f'grid-{grid_count:04}.png'))
                    grid_count += 1
